...
image_icon = PhotoImage(file="logoo1.png")
root.iconphoto(False, image_icon)


frameCnt = 150
frames = [PhotoImage(file='aaaa.gif',format = 'gif -index %i' %(i)) for i in range(frameCnt)]
...
ButtonPause = PhotoImage(file="pausee1.png")
Button(root, image=ButtonPause, bg="#FFFFFF", bd=0, height = 60, width =60,
       command=mixer.music.pause).place(x=300, y=487)

ButtonUrl = PhotoImage(file="codeclause1.png")
Button(root, image=ButtonUrl, bg="#FFFFFF", bd=0, height = 60, width =60,
       command=CodeClause).place(x=385, y=487)
...
Playlist = Listbox(Frame_Music, width=100, font=("Gill Sans MT", 10), bg="#333333", fg="grey", selectbackground="lightblue", cursor="hand2", bd=0, yscrollcommand=Scroll.set)
